...
class Book {
  ...
  post(book = {}) {
    ...
    let raw = this.knex.select().from(tableName).where({ title: title, fbUserId: fbUserId }).limit(2).toSQL()
    return this.knex.raw(raw.sql, raw.bindings).then(async (result) => {
      ...
      try {
        ...
        raw = this.knex
          ...
          .toSQL()
        return this.knex.raw(raw.sql, raw.bindings).then((result) => {
          return Object.assign({}, _book, { cover_url: url }, { id: result[0].insertId })
        })
      } catch (err) {
...
const BookSchema = {
  ...
  get post() {
    return {
      $id: "/Book.post",
      type: "object",
      properties: Util.pick(BookSchema.properties, "current_page", "total_pages", "fbUserId", "title"),
